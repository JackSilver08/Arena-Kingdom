...
export class BotController {
  ...
  private think(engine: MatchEngine) {
    ...
    const midline = GAME_RULES.map.midlineY;
    const intruders = enemyArmy.filter(
      (u) =>
        forwardDir(side) * (u.y - midline) < 0 &&
    ...
    if (this.phase === 'attacking' && target) {
      ...
      if (profile.retreats && this.shouldRetreat(wave, enemyArmy)) {
        const fallback = { x: castle.x, y: castle.y + forwardDir(side) * 110 };
  ...
  private attackTarget(engine: MatchEngine) {
    ...
    const frontline = buildings
      ...
      .sort((a, b) => dir * (a.y - b.y))[0];
    const pick = this.difficulty === 'easy' || !frontline ? castle ?? buildings[0] : frontline;
    return { x: pick.x, y: pick.y - dir * (BUILDING_STATS[pick.type].halfHeight + 16) };
  }
  ...
  private tryBuild(engine: MatchEngine, type: BuildableType) {
    ...
    const frontY = dir > 0 ? bounds.maxY : bounds.minY;
    const candidates: { x: number; y: number }[] = [];
    if (type === 'fence') {
      // Lay segments side by side along the frontline, starting in front of the castle.
      const width = stats.halfWidth * 2;
      for (const k of [0, -1, 1, -2, 2, -3, 3]) candidates.push({ x: castle.x + k * width, y: frontY });
    }
    for (let attempt = 0; attempt < 40; attempt++) {
      const spread = (range: number) => (Math.random() - 0.5) * range;
      if (type === 'village') candidates.push({ x: castle.x + spread(640), y: castle.y + dir * (20 + Math.random() * 150) });
      else if (type === 'barracks') candidates.push({ x: castle.x + spread(460), y: castle.y + dir * (110 + Math.random() * 120) });
      else if (type === 'tower') candidates.push({ x: castle.x + spread(420), y: frontY - dir * (30 + Math.random() * 70) });
      else candidates.push({ x: castle.x + spread(600), y: frontY - dir * Math.random() * 40 });
    }
    candidates.push({
